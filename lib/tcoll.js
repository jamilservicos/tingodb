	...
	fs.open(filename, 'w+', safe.sure(cb, function (fd) {
		...
		safe.eachOfSeries(self._store, function (rec, k, cb) {
			...
			if (!!process.platform.match(/^win/)) {
				...
				// safe way
				fs.rename(filename, self._filename, safe.sure(cb, function () {
					fs.close(self._fd, function(err){ if(err) { cb(err); }  });
					self._fd = fd;
					self._fsize = wpos;
					self._store = store;
					cb();
...
tcoll.prototype.insert = function (docs, opts, cb ) {
    ...
    if(this.isEvents) this.emit('save', docs);
	...
	this._tq.add(function (cb) {
		safe.forEachSeries(docs, function (doc, cb) {
            ...
            if(self.isEvents) self.emit('saved', docs);
			cb(null, docs);
		}))
	}, true, cb)
}
...
tcoll.prototype.update = function (query, doc, opts, cb) {
    ...
    if(self.isEvents) self.emit('save', doc);
	...
	this._tq.add(function (cb) {
		...
		if(query._id) r._id = query._id;
        if(self.isEvents) self.emit('saved', r);
        cb(e,c,r);
	});
}
...
tcoll.prototype.save = function (doc, opts, cb) {
    ...
    if(this.isEvents) this.emit('save', doc);
	...
	this._tq.add(function (cb) {
		...
		(function(cb) {
			...
			self._put(doc, false, safe.sure(cb, function () {
                if(self.isEvents) self.emit('saved', doc);
...
tcoll.prototype.remove = function (query, opts, cb) {
	...
	this._tq.add(function (cb) {
		self.__find(query, null, 0, single ? 1 : null, null, opts.hint, {}, safe.sure(cb, function (res) {
			...
			safe.forEachSeries(res, function (pos, cb) {
				self._get(pos, false, safe.sure(cb, function (obj) {
					if(obj._id) _ids.push({_id: obj._id});

                    /**
					 * Why upgrade? should be the removal of
					 * https://github.com/sergeyksv/tingodb/issues/131
                     console.log(`-->`, obj);
		     */
                ...
                if(self.isEvents) self.emit('removed', _ids);
				cb(null,res.length);
			}))
		}))
	},true,cb);
...
tcoll.prototype.publish = function(event, data){
	var self = this;
	this.isEvents = false;
    this.insert({
        eventName: event,
    	...
    	var _msg = r[0].msg;
        // _msg._id = r[0]._id;
		self.emit(event, _msg);

        self.remove({_id: {$lt: r[0]._id - 1000}});
	});
};

tcoll.prototype.unsubscribe = function(event){
